...
class Agent():
    ...
    def __init__(self, state_size, action_size, agent_num, random_seed=2):
        ...
        # Noise process
        self.noise = OUNoise(agent_num, action_size, random_seed)
    ...
    def act(self, state, add_noise=True):
        ...
        with torch.no_grad():
            for i in range(self.agent_num):
                actions[i, :] = self.actor_local(state[i]).cpu().data.numpy()
...
class OUNoise:
    ...
    def __init__(self, batch_size, size, seed, mu=0., theta=0.15, sigma=0.1):
        """Initialize parameters and noise process."""
        
        self.batch_size = batch_size
        self.size = size
        
        self.mu = mu * np.ones((batch_size, size))
        self.theta = theta
        self.sigma = sigma
    ...
    def sample(self):
        ...
        dx = self.theta * (self.mu - x) + self.sigma * np.random.normal(0,1,(self.batch_size, self.size))
        self.state = x + dx
        return self.state
